...
title=$(echo "$html_content" | grep -oP '(?<=<meta property="og:title" content=").*?(?=")' | xml unesc) 

thumbnail=$(echo "$html_content" | grep -oP '(?<=<meta property="og:image" content=").*?(?=")')

description=$(echo "$html_content" | grep -oP '(?<=<meta property="og:description" content=").*?(?=")' | xml unesc)
...
channel=$(echo "$html_content" | grep -oP '(?<=<link itemprop="name" content=").*?(?=")' | xml unesc)
